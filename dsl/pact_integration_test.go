...
func TestPact_Integration(t *testing.T) {
	...
	if os.Getenv("PACT_INTEGRATED_TESTS") != "" {
		...
		// Pass in test case
		var test = func() error {
			_, err := http.Get(fmt.Sprintf("http://localhost:%d/foobar", pact.ServerPort))
			if err != nil {
				t.Fatalf("Error sending request: %v", err)
			}
			_, err = http.Get(fmt.Sprintf("http://localhost:%d/bazbat", pact.ServerPort))
			if err != nil {
				t.Fatalf("Error sending request: %v", err)
			}
		...
		body :=
			map[string]interface{}{
				"items": ArrayMinLike(1,
					map[string]interface{}{
						"size":   Like(10),
						"tag":    ArrayMinLike(2, []interface{}{Like("jumper"), Like("shirt")}),
		...
		pact.
			...
			WillRespondWith(Response{
				Status: 200,
				Body:   PactBodyBuilder(body),
			})

		// Verify Collaboration Test interactionns (Consumer side)
